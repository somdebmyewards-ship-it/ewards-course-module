...
class ChatbotController extends Controller
{
    ...
    private function hfSimilarity(string $question, array $sentences): array
    {
        try {
            ...
            $response = Http::withHeaders(['Authorization' => "Bearer {$token}"])
                ->timeout(20)
                ->post('https://router.huggingface.co/hf-inference/models/sentence-transformers/all-MiniLM-L6-v2/pipeline/sentence-similarity', [
                    'inputs' => [
    ...
    // ── Abuse & greeting detection ────────────────────────────────────

    /**
     * Returns a response array if the question is abusive/off-topic, otherwise null.
     */
    private function detectAbuse(string $question): ?array
    {
        ...
        foreach ($abusivePatterns as $pattern) {
            if (preg_match($pattern, $question)) {
                return [
                    'answer'       => "Let's keep things professional! 😊 I'm here to help you with eWards features. Ask me anything about Campaigns, Instant Pass, Customer Management, and more.",
                    'sources'      => [],
                    'suggestions'  => $this->defaultSuggestions(),
                    'answer_found' => false,
                ];
        ...
        foreach ($injectionPatterns as $pattern) {
            if (preg_match($pattern, $question)) {
                return [
                    'answer'       => "I noticed something unusual in your question. I'm only able to answer questions about eWards training content.",
                    'sources'      => [],
                    'suggestions'  => $this->defaultSuggestions(),
                    'answer_found' => false,
                ];
    ...
    /**
     * Returns a response array for greetings/small-talk so we skip the retrieval pipeline.
     * Returns null for real questions.
     */
    private function detectGreeting(string $question): ?array
    {
        $lower = trim(strtolower($question));

        $greetings = [
            '/^(hi|hello|hey|howdy|hiya|yo|sup|good\s+(morning|afternoon|evening|day))[\s!?.]*$/i',
        ...
        foreach ($greetings as $pattern) {
            if (preg_match($pattern, $lower)) {
                return [
                    'answer'       => "Hi there! 👋 I'm Ela, your eWards learning assistant. I can help you understand the platform — ask me about **Campaigns**, **Instant Pass**, **Customer Upload**, **Rewards**, or **Reports**. What would you like to know?",
                    'sources'      => [],
                    'suggestions'  => $this->defaultSuggestions(),
                    'answer_found' => true,
                ];
        ...
        $farewells = [
            '/^(bye|goodbye|see\s+you|thanks?\s*(bye)?|thank\s+you|cheers|ok\s+thanks?|great\s+thanks?)[\s!?.]*$/i',
        ];

        foreach ($farewells as $pattern) {
            if (preg_match($pattern, $lower)) {
                return [
                    'answer'       => "You're welcome! 😊 Come back anytime you have questions about eWards. Happy learning!",
                    'sources'      => [],
                    'suggestions'  => $this->defaultSuggestions(),
                    'answer_found' => true,
                ];
