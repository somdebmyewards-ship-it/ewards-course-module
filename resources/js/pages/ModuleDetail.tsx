    ...
    <div style={{ maxWidth: '100%' }}>
      ...
      <Layout style={{ background: 'transparent', minHeight: 500 }}>
        ...
        <Content style={{ background: 'transparent', minWidth: 0 }}>
          ...
          {/* Hidden pre-loader: mounts the iframe as soon as the module page
              opens so clicking "Interactive Practice" feels instant instead of
              waiting for a fresh HTML load. Same URL, off-screen, no layout. */}
          {hasPrototype && mod.prototype_url && !viewingPractice && (
            <iframe
              ...
              tabIndex={-1}
              style={{
                position: 'absolute', width: 1, height: 1, left: -9999, top: -9999,
                border: 'none', opacity: 0, pointerEvents: 'none',
              }}
            />
          )}

          {/* Interactive Prototype — only when the user clicks "Interactive Practice" in sidebar */}
          {hasPrototype && !viewingIntro && viewingPractice && step === 0 && (
            <div id="prototype-practice-card" style={{
              ...
              {mod.prototype_url ? (
                // Prototype now has 400px chat-area (down from 520px), no
                // `min-height: 100vh` on body, and flex-wrap on main-layout.
                // 1100px covers both side-by-side and wrap-stack modes.
                <iframe
                  src={mod.prototype_url}
          ...
          {step === 0 && !viewingPractice && (<>
              ...
              >
                {hasPrototype && !progress?.prototype_completed ? 'Finish Practice to Unlock Quiz' : 'Continue to Quiz →'}
              </button>}
            </>)}
